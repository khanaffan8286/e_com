						...
						<div class="row mb-10">
							<div class="col-lg-12">
								<div class="group-button">
									<button id="orderstatus_0" type="button" onClick="onDataViewByStatus(0)" class="btn btn-theme orderstatus active">{{ __('All') }} (@php echo OrderCountForSeller(0); @endphp)</button>
									@foreach($order_status_list as $row)
									<button id="orderstatus_{{ $row->id }}" type="button" onClick="onDataViewByStatus({{ $row->id }})" class="btn btn-theme orderstatus">{{ $row->ostatus_name }} (@php echo OrderCountForSeller($row->id); @endphp)</button>
									@endforeach
								</div>
